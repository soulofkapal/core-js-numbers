...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const cardinate1 = x2 - x1;
  const cardinate2 = y2 - y1;
  return Math.sqrt(cardinate1 * cardinate1 + cardinate2 * cardinate2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorV = Math.sqrt(x1 * x1 + y1 * y1);
  const vectorU = Math.sqrt(x2 * x2 + y2 * y2);
  const vectorR = vector1 / (vectorV * vectorU);
  return Math.acos(vectorR);
}
...
function getCountOfOddNumbers(number) {
  let x = 0;
  const sortn = Math.abs(number);
  for (let i = 0; i <= sortn; i += 1) {
    if (i % 2 !== 0) {
      x += 1;
    }
  }
